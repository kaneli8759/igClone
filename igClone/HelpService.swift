    ...
    static func sendDataToDatabase(photoUrl: String, videoUrl: String? = nil, ratio: CGFloat, caption: String, onSuccess: @escaping () -> Void) {
        ...
        for var word in words {
            if word.hasPrefix("#") {
                ...
                let newHashTagPost = [newPostId: true]
        ...
        newPostReference.setValue(dict, withCompletionBlock: {
            ...
            let postRef = ["timestamp": timestamp]
            Api.Feed.REF_FEED.child(Api.User.CURRENT_USER!.uid).child(newPostId!).setValue(postRef)
            Api.Follow.REF_FOLLOWERS.child(Api.User.CURRENT_USER!.uid).observeSingleEvent(of: .value, with: {
                snapshot in
                let arraySnapshot = snapshot.children.allObjects as! [DataSnapshot]
                arraySnapshot.forEach({ (child) in
                    print(child.key)
                    Api.Feed.REF_FEED.child(child.key).child(newPostId!).setValue(postRef)
            ...
            myPostRef.setValue(postRef, withCompletionBlock: { (error, ref) in
                if error != nil {
                    print(error!.localizedDescription)
                    return
                }
